...
- El esquema de validación está en `src/config/env.ts` (Zod). `DATABASE_URL` y
  ...
  (ADR-0015 §5): `database: "ok"` si la base responde y tiene el esquema migrado, `unreachable`
  (HTTP 503) si está definida pero no responde en 2 s, `unmigrated` (HTTP 503) si responde pero le
  falta el esquema o el historial de migraciones, y `unconfigured` (HTTP 200) si falta la variable o
