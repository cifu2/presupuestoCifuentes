 ...
 */
const DRAFT_WITHOUT_OVERLAP = '0192f1b0-0000-7000-8000-000000000401'
...
test.describe('sin ADMIN_API_TOKEN, el endpoint de publicación', () => {
  test('falla cerrado con 503 y no llega a mirar la versión', async ({ request }) => {
    ...
    for (const tariffVersionId of [DRAFT_WITHOUT_OVERLAP, MISSING_TARIFF]) {
...
test.describe('con ADMIN_API_TOKEN, el endpoint de publicación', () => {
  ...
  test('rechaza con 401 las credenciales ausentes o incorrectas', async ({ request }) => {
    const withoutCredentials = await request.post(publishPath(DRAFT_WITHOUT_OVERLAP))

    expect(withoutCredentials.status()).toBe(401)
    expect((await withoutCredentials.json()).error.code).toBe('UNAUTHORIZED')

    const withWrongToken = await request.post(publishPath(DRAFT_WITHOUT_OVERLAP), {
  ...
  test('publica un borrador sin solape y repetirlo es idempotente', async ({ request }) => {
    const first = await request.post(publishPath(DRAFT_WITHOUT_OVERLAP), {
      headers: withToken(E2E_ADMIN_TOKEN),
    })
    const published = (await first.json()).data
    ...
    const repeated = await request.post(publishPath(DRAFT_WITHOUT_OVERLAP), {
      headers: withToken(E2E_ADMIN_TOKEN),
    })
